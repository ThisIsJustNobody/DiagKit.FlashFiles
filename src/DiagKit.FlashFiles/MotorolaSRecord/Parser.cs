...
internal static class Parser
{
    ...
    private static List<FlashBlock> ParseCore(IEnumerable<string> lines, byte dataSize, bool checkValid)
    {
        ...
        foreach (var line in lines)
        {
            ...
            switch (record.RecordType)
            {
                ...
                case RecordType.S2:
                case RecordType.S3:
                    ParserBlockBuilder.AddSegment(segments, record.Address, record.Data, dataSize, lineNumber);
                    dataRecordCount++;
                    dataSeen = true;
                    break;
                ...
                case RecordType.S5:
                case RecordType.S6:
                    if (record.Address != dataRecordCount)
    ...
    private static void ValidateRecord(RecordItemSlim record, bool checkValid, int lineNumber)
    {
        ...
        if (checkValid && record.CheckSum != record.CalculatedCheckSum)
